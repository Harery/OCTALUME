# Example: ShopEase Sprint 1 Plan

**Document ID:** P4-SPRINT-001  
**Sprint Number:** 1  
**Sprint Duration:** 2026-02-03 - 2026-02-14 (2 weeks)  
**Author:** Maria Santos, Scrum Master  
**Traceability:** Links to P4-WBS-001

---

## Sprint Goal

**Deliver user authentication and basic product catalog browsing capabilities.**

Users should be able to register, log in, and browse product categories by sprint end.

---

## Sprint Metrics

| Metric | Value |
|--------|-------|
| Team Velocity (historical) | 42 points |
| Committed Points | 38 points |
| Team Capacity | 320 hours |
| Sprint Duration | 10 working days |

---

## Sprint Backlog

### User Stories

| ID | Story | Points | Assignee | Status |
|----|-------|--------|----------|--------|
| US-001 | As a user, I want to register with email so I can create an account | 5 | Alex Chen |  Done |
| US-002 | As a user, I want to login with my credentials so I can access my account | 5 | Alex Chen |  Done |
| US-003 | As a user, I want to reset my password so I can recover account access | 3 | Alex Chen |  Done |
| US-004 | As a user, I want to browse product categories so I can discover products | 8 | Jordan Lee |  Done |
| US-005 | As a user, I want to view a product listing page so I can see available items | 8 | Jordan Lee |  Done |
| US-006 | As a user, I want to view product details so I can learn about items | 5 | Sam Wilson |  Done |
| US-007 | As a user, I want to sort products by price so I can find deals | 3 | Sam Wilson | 👀 In Review |
| US-008 | As a user, I want basic product filtering so I can narrow my search | 5 | Sam Wilson | ⬜ Carryover |

**Completed:** 33/38 points (87%)

---

## Team Capacity

| Team Member | Role | Days | Hours | Allocated |
|-------------|------|------|-------|-----------|
| Alex Chen | Senior Dev | 10 | 80 | 72 |
| Jordan Lee | Developer | 10 | 80 | 80 |
| Sam Wilson | Developer | 8 | 64 | 64 |
| Taylor Kim | QA Engineer | 10 | 80 | 60 |
| **Total** | | | **304** | **276** |

---

## Sprint Outcome

### Velocity

| Metric | Planned | Actual |
|--------|---------|--------|
| Story Points | 38 | 33 |
| Stories Completed | 8 | 6 |
| Sprint Goal | Partial | 87% |

### Retrospective Actions

| Action | Owner | Due |
|--------|-------|-----|
| Improve estimation for complex stories | Jordan | Sprint 2 |
| Add acceptance criteria earlier | Maria | Sprint 2 |

---

**Sprint Completed:** 2026-02-14

---

**Document End**

---

**Version 1.0.0 | OCTALUME Enterprise Lifecycle Framework**
